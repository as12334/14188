...
/*抽奖板面*/
	public function luckyshow() {
		...
		include templates("index","luckyshow");
	...
	public function luckylist() {
		...
		$luckylist=$this->db->GetList("select * from  `@#_member_lucky`  where `uid`='$uid' and (type=2) ORDER BY `id` DESC limit 0,30");
		if(!empty($luckylist)){
		  $recount=1;
		}
		include templates("index","luckylist");
	...
	public function luckynei() {
		...
		include templates("index","luckynei");
	...
	public function shouhuo() {
		...
		header("location:".WEB_PATH."/go/lucky/luckylist/");
	...
	public function record(){
		...
		$recordarr=$mysql_model->GetList("select * from  `@#_member_cashout`  where `uid`='$uid' and type=2 ORDER BY `time` DESC limit 0,40");
        
		if(!empty($recordarr)){
		  $recount=1;
		}		
		include templates("index","record");
	}
		//现金充值到账户
	public function useryj(){
		...
		include templates("index","useryj");
	}
	function cashout(){
