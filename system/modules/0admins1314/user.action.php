...
class user extends admin {
	...
	public function __construct(){		
		...
		$this->ment=array(
						array("lists","管理员管理",ROUTE_M.'/'.ROUTE_C."/lists",'hide'),
						array("reg","添加管理员",ROUTE_M.'/'.ROUTE_C."/reg",'hide'),
						array("edit","修改管理员",ROUTE_M.'/'.ROUTE_C."/edit",'hide'),
		);
	...
	public function reg(){
       ...
       if($info['mid'] == 0 ||  $admins){}else{exit();}
		if(isset($_POST['submit-1'])){
			...
			$addtime=time();
			$ip=_get_ip();
			$this->db->Query("INSERT INTO `@#_admin` (`mid`, `username`, `userpass`, `useremail`, `addtime`, `logintime`, `loginip`) VALUES ('$pmid', '$username', '$password', '$aa','$addtime','0','$ip')");
			if($this->db->affected_rows()){			
				$path=WEB_PATH.'/'.ROUTE_M.'/user/lists';
				_message("添加管理员成功!",$path);
			}else{
				_message("添加管理员失败!",$paths);
			}
		}
	...
	public function editpass(){
		...
		$admin=$this->db->GetOne("SELECT * FROM `@#_admin` WHERE `uid`='$uid'");
		if(!is_array($admin)){
		...
		if(isset($_POST['submit-1'])){
			$password0=$_POST['passwordold'];			
			$password1=$_POST['password'];
			$password2=$_POST['pwdconfirm'];
			$email=isset($_POST['email'])?($_POST['email']):'';
			
			if(($admin['userpass']!=md5($password0))){
				_message("原密码不对!");
			}
			...
			include $this->tpl(ROUTE_M,'user.editpass');
		}
	}
	public function edit(){
       ...
       if($info['mid'] == 0 ||  $admins){}else{exit();}
		...
		if(isset($_POST['submit-1'])){			
			...
			$password=md5($password2);
			$ok = $this->db->Query("UPDATE `@#_admin` SET `useremail`='$aa' WHERE (`uid`='$uid')");
			if($ok){
			_message("修改成功!",$path);	
			}else{
				_message("修改失败!");
	...
	public function out(){
			_setcookie("AID",'');
			_setcookie("ASHELL",'');
			header("location:".G_MODULE_PATH."/user/login");
