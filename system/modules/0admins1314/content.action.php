...
HTML;
	...
	public function goods_edit(){
		...
		if(isset($_POST['dosubmit'])){		
			...
			if($this->db->Query($sql)){			
				$this->db->Autocommit_commit();	
				$rowid=$this->db->GetOne("SELECT * FROM `@#_shoplist` WHERE `id` = '$shopid' LIMIT 1");
				$cateid = intval($rowid['cateid']);
				$rowparentid=$this->db->GetOne("SELECT * FROM `@#_category` WHERE `cateid` = '$cateid' LIMIT 1");
			    $parentids = intval($rowparentid['parentid']);
				
				$this->db->Query("UPDATE `@#_shoplist` SET `parentid` = '$parentids' where `id` = '$shopid'");
				_message("修改成功!",WEB_PATH.'/'.ROUTE_M.'/content/goods_list/');
			}else{	
				$this->db->Autocommit_rollback();
				_message("修改失败!".$s_sid);
			}			
	...
	$dealerList=$this->db->GetList("SELECT * FROM `@#_member_dealer` where pass=2");
	$info=$this->AdminInfo;
	...
	public function goods_add(){		
		...
		if(isset($_POST['dosubmit'])){		
		    $cateid = intval($_POST['cateid']);
		    $rowparentid=$this->db->GetOne("SELECT * FROM `@#_category` WHERE `cateid` = '$cateid' LIMIT 1");
			$parentid = intval($rowparentid['parentid']);
			...
			$query_1 = $this->db->Query("INSERT INTO `@#_shoplist` (`parentid`,`zhiding`,`cateid`, `brandid`, `title`, `title_style`, `title2`, `keywords`, `description`, `money`, `yunjiage`, `zongrenshu`, `canyurenshu`,`shenyurenshu`, `qishu`,`maxqishu`,`thumb`, `picarr`, `content`,`xsjx_time`,`renqi`,`pos`,`bannershop`,`renqipos`,`newpos`,`posthumb`, `time`, `auto_money`) VALUES ('$parentid','".htmlspecialchars($_POST['zhiding'])."','$cateid', '$brandid', '$title', '$title_style', '$title2', '$keywords', '$description', '$money', '$yunjiage', '$zongrenshu', '$canyurenshu','$zongrenshu', '1','$maxqishu', '$thumb', '$picarr', '$content','$xsjx_time','$goods_key_renqi', '$goods_key_pos','$goods_key_bannershop','$renqipos','$newpos','$posthumb','$time','$auto_money')");
			
			if($orders==1){
				$pro_can1 = $money*2;
				$query_0 = $this->db->Query("INSERT INTO `pro_co` (`lm`,`url`,`title`,`pro_can1`,`pro_can2`,`pro_can3`,`pro_can4`,`f_body`,`z_body`,`img_sl0`,`img_sl1`,`img_sl2`,`img_sl3`,`img_sl4`,`img_sl5`,`px`,`read_num`,`pass`,`tuijian`,`hot`,`wtime`,`ip`,`times`,`ttime`,`type`,`user_admin`,`user_member`,`styles`,`cart_num`,`cart_title`,`cart_img`,`cart_code`,`order_num`,`fhd`,`ems`) VALUES ('$parentid','', '$title', '$pro_can1', '$money',900, '$title2', '','$content', '$thumb', '', '', '', '','', '1','0', 'yes', 'yes', 'no','$time','', '','','特卖','','',0,1,'图片颜色','', '', '', '广东省 广州市', '韵达快递')");
				}
	...
	public function goods_w_add(){		
		...
		if(isset($_POST['dosubmit'])){		
			...
			$keywords = htmlspecialchars($_POST['keywords']);
			$cart_title = htmlspecialchars($_POST['description']);
			...
			$posthumb = trim(htmlspecialchars($_POST['posthumb']));	
			 ...
			 $fhd = $_POST['fhd'];			
			 $ems = $_POST['ems'];	
			 $pro_can1 = $_POST['pro_can1'];
			 
			 $row=$this->db->GetOne("SELECT * FROM `pro_co` order by `id` desc");
			 
			 $num = $row['id']+1;
			 $ex_title = explode(" ", trim($cart_title));  
$count_title = count($ex_title);
...
for($i=0;$i<=$count_title-1;$i++){
	...
	$cart_num.=$num.$i." ";
			...
			$time=time();	//商品添加时间		
				...
				$query_0 = $this->db->Query("INSERT INTO `pro_co` (`lm`,`url`,`title`,`pro_can1`,`pro_can2`,`pro_can3`,`pro_can4`,`f_body`,`z_body`,`img_sl0`,`img_sl1`,`img_sl2`,`img_sl3`,`img_sl4`,`img_sl5`,`px`,`read_num`,`pass`,`tuijian`,`hot`,`wtime`,`ip`,`times`,`ttime`,`type`,`user_admin`,`user_member`,`styles`,`cart_num`,`cart_title`,`cart_img`,`cart_code`,`order_num`,`fhd`,`ems`) VALUES ('$parentid','', '$title', '$pro_can1', '$money',900, '$title2', '','$content', '$thumb', '', '', '', '','', '1','0', 'yes', 'yes', 'no','$time','', '','','特卖','','',0,'$cart_num','$cart_title','', '', '', '$fhd', '$ems')");
			
			
					
			if($query_0){
							
				_message("商品添加成功!",WEB_PATH.'/'.ROUTE_M.'/content/goods_w_lists/');
			}else{		
		...
		$dealerList=$this->db->GetList("SELECT * FROM `@#_member_dealer` where pass=2");
		include $this->tpl(ROUTE_M,'shop.w_insert');
			
	}
	
	//微商城修改商品
	public function goods_w_edit(){	
	  $shopid=intval($this->segment(4));	
	  $shopinfo=$this->db->GetOne("SELECT * FROM `pro_co` WHERE `id` = '$shopid' LIMIT 1");	
...
for($i=0;$i<=$count_title-1;$i++){
	...
	$cart_num.=$num.$i." ";
			...
			$sql='update `'.$tablepre.'pro_co` set `url`="",`title`="'.$title.'",`pro_can1`="'.$pro_can1.'",`pro_can2`="'.$money.'",`pro_can3`="'.$pro_can3.'",`pro_can4`="'.$title2.'",`z_body`="'.$content.'",`img_sl0`="'.$thumb.'",`cart_num`="'.$cart_num.'",`cart_title`="'.$cart_title.'",`fhd`="'.$fhd.'",`ems`="'.$ems.'" where `id`='.$id.'';

			
				
				$query_0 = $this->db->Query($sql);
			
			
					
			if($query_0){
							
				_message("商品修改成功!",WEB_PATH.'/'.ROUTE_M.'/content/goods_w_lists/');
			}else{		
		...
		include $this->tpl(ROUTE_M,'shop.w_edit');
			
	}
	
	//微商城商品列表
	public function goods_w_lists(){
		$info=$this->AdminInfo;
		$shoplist=$this->db->GetList("SELECT * FROM `pro_co`");
		$total=$this->db->GetCount("SELECT COUNT(*) FROM pro_co");
		include $this->tpl(ROUTE_M,'shop.w_lists');
	}
	
	//微商城删除商品
	public function goods_w_del(){
	  $shopid=intval($this->segment(4));	
	  $query_0 = $this->db->Query("DELETE FROM `pro_co` WHERE (`id`='$shopid') LIMIT 1");
	  
	  if($query_0){
							
				_message("商品删除成功!",WEB_PATH.'/'.ROUTE_M.'/content/goods_w_lists/');
			}else{		
	...
	public function goods_dels(){
		...
		$q0 = $this->db->Query("INSERT INTO `@#_shoplist_del` select * from `@#_shoplist` where `id` = '$shopid'");
		$this->db->Autocommit_start();	
		foreach($info as $k=>$v){
			$id    = $v['id'];
			$sid   = $v['sid'];
	...
	*/	
	public function goods_max_qishu(){
		...
		if($money < $onemoney){
			echo json_encode(array("meg"=>" 总价不能小于抢购价!","err"=>"-1"));
			exit;
		}
	...
	**/
	public function goods_set_money(){		
		...
		if(isset($_POST['money']) || isset($_POST['yunjiage'])){
			...
			$info =$this->db->GetList("SELECT * from `@#_member_go_record` WHERE `shopid` = '$gid'");
		...
		foreach($info as $k=>$v){
			...
			$q5 = $this->db->Query("UPDATE `@#_member` SET `money` = `money` + $money where `uid` = '$uid'");
		}
