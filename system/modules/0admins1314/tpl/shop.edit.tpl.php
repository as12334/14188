	...
	<table width="100%"  cellspacing="0" cellpadding="0">
    	<tr style="background-color:#FFe;height:50px;">
			...
			<td>
				...
				单次价格	<b style="color:red"><?php echo $shopinfo['yunjiage']; ?></b>&nbsp;&nbsp;&nbsp;
        ...
        <tr>
         ...
         <!------20150901增加----------------> 
         <tr>
			<td align="right" style="width:120px"><font color="red">*</font>商品推广二维码：</td>
			<td>
           <img src="http://qr.topscan.com/api.php?bg=ffffff&fg=000000&el=l&w=200&m=10&text=<?php echo WEB_PATH.'/mobile/mobile/item/'.$shopinfo['id']; ?>"/>
         ...
         <tr>
			...
			<td>
            <input   type="text" id="zhiding" value="<?php echo $shopinfo['zhiding']; ?>" name="zhiding" class="input-text" style="width:100px; text-align:center" onKeyUp="value=value.replace(/\D/g,'')">  *为空或0表示不指定       
            </td>
		</tr>  
        
        <tr>      
			<td align="right" style="width:120px">自动约定中奖会员：</td>     
            <td><input type="text" name="auto_money" value="<?php echo $shopinfo['auto_money'];?>" class="input-text" style="width:100px; text-align:center" onKeyUp="value=value.replace(/\D/g,'')">当已刷的总数大于<?php echo $shopinfo['auto_money'];?>就自动约定</td>
		</tr>  
        <?php }?>     
<!------20150901增加结束---------------->    
        <tr>
        ...
        <tr>
			<td align="right" style="width:120px">副标题：</td><?php echo $shopinfo['title_style']; ?>
			<td><input  type="text" value="<?php echo $shopinfo['title2']; ?>" style="<?php echo $shopinfo['title_style']; ?>" name="title2" id="title2" onKeyUp="return gbcount(this,100,'texttitle2');"  class="input-text wid400" maxlength="100">
        ...
        <tr>
			...
			<td><select name="description" class=" wid300"> 
            <?php foreach($dealerList as $v){
			$membshop = $this->db->GetOne("SELECT * FROM `@#_member_dealer` WHERE `shopid` = '$v[shopid]' LIMIT 1");
		
		    if($info['uid']==1){
			?>
            
            <option value="<?php echo $v['shopname']; ?>" <?php if($v['shopname'] == $shopinfo['description']){ ?> selected="selected" <?php }?>><?php echo $v['shopname']; ?></option>
