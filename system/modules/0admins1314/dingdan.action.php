...
class dingdan extends admin {
	...
	public function __construct(){		
		...
		$infomob  = $this->AdminInfo;
		$adminuid = $infomob['uid'];
		if($adminuid==1){	
		$this->ment=array(
						...
						array("lists","真客户订单",ROUTE_M.'/'.ROUTE_C."/lists/auto_user"),
						array("lists","真客户中奖",ROUTE_M.'/'.ROUTE_C."/lists/auto_user_zj"),
						array("lists","真客户未发货",ROUTE_M.'/'.ROUTE_C."/lists/auto_user_notsend"),
		);
		}else{
		$this->ment=array(
						array("lists","订单列表",ROUTE_M.'/'.ROUTE_C."/lists"),					
	...
	public function lists(){	
		$infomob  = $this->AdminInfo;
		$adminuid = $infomob['mid'];
		$username = $infomob['username'];
		...
		*/
		...
		}elseif($where == 'auto_user'){
			//真实客户			
			$list_where = "where `auto_user`=0";
		}elseif($where == 'auto_user_zj'){
			//真客户中奖			
			$list_where = "where `auto_user`=0 and `huode` != '0'";
		}elseif($where == 'auto_user_notsend'){
			//真客户未发货			
			$list_where = "where `auto_user`=0 and `huode` != '0' and `status` LIKE  '%未发货%'";		
		}
