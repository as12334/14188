...
class member extends admin {
	...
	*/
	public function lists(){
		...
		$user_type_arr = array("def"=>"默认会员","noreg"=>"未认证会员","auto_user"=>"真实会员","day_new"=>"今日新增","day_shop"=>"今日消费","b_qq"=>"QQ绑定会员","b_weibo"=>"微博绑定会员","b_taobao"=>"淘宝绑定会员");
		if(!isset($user_type_arr[$user_type])){
		//	$user_type = "def";		
		}		
		if($user_type == 'del'){$table = "@#_member_del";}else{$table = "@#_member";}
		
		$user_ziduan_arr = array("uid"=>"会员ID","money"=>"账户金额","score"=>"账户微积分","jingyan"=>"会员经验","time"=>"注册时间","login_time"=>"登陆时间");	
		...
		$num=20;
		$total=$this->db->GetCount("SELECT COUNT(*) FROM `$table` WHERE time!='' and $users and $sql_where"); 
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");		

		$members=$this->db->GetPage("SELECT * FROM `$table` WHERE time!='' and $users and $sql_where order by `$user_ziduan` $user_order",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
	...
	public function insert(){
		...
		if(isset($_POST['submit'])){
			...
			$info=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `username` ='$username' and `username` !=''  LIMIT 1");	
			if($info){
				_message("该用户名已注册!");
			}
			$infoe=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `email` ='$email' and `email` !='' LIMIT 1");	
			if($infoe){
				_message("该邮箱已注册!");
			}
			$infom=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `mobile` ='$mobile' and `mobile` !=''  LIMIT 1");	
			if($infom){
				_message("该手机已注册!");
			}
			if(empty($password)){
			...
			$sql="INSERT INTO `@#_member` (username,img,email,mobile,user_ip,password,money,jingyan,score,emailcode,mobilecode,qianming,groupid,time,auto_user) value ('$username','$img','$email','$mobile','$user_ip','$password','$money','$jingyan','$score','$emailcode','$mobilecode','$qianming','$membergroup','$time',1)";
			$this->db->Query($sql);
	...
	public function modify(){
		...
		$member_account_list=$this->db->Getlist("select * from `@#_member_account`  where `uid`='$uid' order by id desc");
		$member_go_record=$this->db->Getlist("select * from `@#_member_go_record`  where `uid`='$uid' order by id desc");
		$member_lucky=$this->db->Getlist("select * from `@#_member_lucky`  where `uid`='$uid' order by id desc");
		//$member_money=$this->db->Getone("select sum(money) from `@#_member_account`  where `uid`='$uid'");
		...
		if(isset($_POST['submit'])){
			...
			$infom=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `mobile` ='$mobile' and `mobile` !=''  LIMIT 1");	
			if($mobile!=$infom['mobile'] || $member['mobile']==""){
				if($infom){
				_message("该手机已注册!");
			 }
			}
			...
			$sql="UPDATE `@#_member` SET `username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`money`='$money',`jingyan`='$jingyan',`score`='$score',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`groupid`='$membergroup',`qianming`='$qianming',`user_ip`='$user_ip' WHERE `uid`='$uid'";
	...
	//会员组
		...
		public function member_fufen(){
			if(isset($_POST['submit'])){			
				...
				$u_reg=intval(trim($_POST['u_reg']));
				//以上是微积分，一下是经验值
				$z_reg=intval(trim($_POST['z_reg']));
				...
				$fufen_yongjin_2=floatval(trim($_POST['fufen_yongjin_2']));
				$fufen_yongjin_3=floatval(trim($_POST['fufen_yongjin_3']));
				$fufen_yongjin_4=floatval(trim($_POST['fufen_yongjin_4']));
				
				$u_jf=intval(trim($_POST['u_jf']));
...
return array (	
	...
	'fufen_yongjin_2' => '$fufen_yongjin_2',
	'fufen_yongjin_3' => '$fufen_yongjin_3',
	'fufen_yongjin_4' => '$fufen_yongjin_4',
...
HTML;
	...
	public function commissions(){	
		...
		if(isset($_GET['p'])){$pagenum=$_GET['p'];}else{$pagenum=1;}	
		$page->config($total,$num,$pagenum,"0");		
		$commissions=$this->db->GetPage("SELECT * FROM `@#_member_cashout`  WHERE 1 order by id desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		
		//查询用户名
		if(!empty($commissions)){
		   foreach($commissions as $key=>$val){
		      $uid=$val['uid'];
			  $user[$key]=$this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$uid'  ");
	...
	public function recharge(){
		...
		if(isset($_POST['sososubmit'])){
			...
			if(empty($chongzhi) || $chongzhi=='请选择充值来源') $content=" AND (`content`='充值' or `content`='管理员修改金额' or `content`='使用佣金充值到用户账户' or `content`='使用余额宝充值到用户账户' or `content`='通过微信公众号充值')";
		...
		$num=20;
		if(empty($wheres)){
			$total=$this->db->GetCount("SELECT count(*) FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到用户账户' or `content`='使用余额宝充值到用户账户' or `content`='通过微信公众号充值') AND `type`='1' and money>=1 "); 
			$summoeny=$this->db->GetOne("SELECT sum(money) sum_money FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到用户账户' or `content`='使用余额宝充值到用户账户' or `content`='通过微信公众号充值') AND `type`='1'"); 
		}else{
		...
		$page->config($total,$num,$pagenum,"0");	
		if(empty($wheres)){
			$recharge=$this->db->GetPage("SELECT * FROM `@#_member_account` WHERE (`content`='充值' or `content`='使用佣金充值到用户账户' or `content`='使用余额宝充值到用户账户' or `content`='通过微信公众号充值') AND `type`='1' and money>=1 order by `time` desc",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0)); 
		}else{
