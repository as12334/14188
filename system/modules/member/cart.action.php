...
class cart extends base {
	...
	public function pay(){
		...
		$sq = "and (pay_class!='wapalipay' and pay_class!='wxpay_web')";
		$paylist = $this->db->GetList("select * from `@#_pay` where `pay_start` = '1' $sq");
		...
		//微信支付
		$payid  = 8;
		$payapp = $this->db->GetOne("SELECT * FROM `@#_pay` where `pay_id` = '$payid'");
		$titleconfig = $this->db->GetOne("SELECT * FROM `@#_config` where `id` = '23'");
		
		
		
		$payarry = explode('"',$payapp['pay_key']);
		$app_id = $payarry[9];
		$app_secret = $payarry[19];

        $title = $titleconfig['value']."订单";
        $amount = $MoenyCount*100;//支付总价
        $out_trade_no = "abc" . time();//订单号，需要保证唯一性
       //1.生成sign
	...
	public function paysubmit(){
		$type_pay_list=isset($_POST['type_pay_list']) ? $_POST['type_pay_list'] : "/mobile/cart/cartlist";
		$type_pay_success=isset($_POST['type_pay_success']) ? $_POST['type_pay_success'] : "/mobile/cart/paysuccess";
		
		if(!isset($_POST['submit'])){	
			_message("正在返回购物1车...",WEB_PATH.$type_pay);
			exit;
		}	
		...
		if(isset($_POST['submitcode'])) {
			...
			if($_POST['submitcode'] == $submitcode){			
				...
				_message("请不要重复提交...",WEB_PATH.$type_pay_list);
			}	
		}else{
			_message("正在返回购物车...",WEB_PATH.$type_pay_list);
		}
		...
		_setcookie("wuid",($uid),60*60*24*7);
		...
		if(isset($_POST['shop_score'])){ //使用福分
		...
		*************/
		...
		$ok = $pay->init($uid,$pay_type_id,'go_record');	//微购商品	
		if($ok !== 'ok'){
		...
		if($check === 'not_pay'){
			_message('未选择支付平台!',WEB_PATH.$type_pay_list);
		}
		if(!$check){
            ...
            $auto_ok = $this->db->Query("UPDATE `@#_shoplist` SET `xsjx_time` = '$xsjx_times' where `shenyurenshu` < 3");
		
			_message("对不起，手慢，没抢到!",WEB_PATH.$type_pay_list);
		}
		if($check){
			
			
			//成功
			header("location: ".WEB_PATH.$type_pay_success);
		}else{
			//失败	
			$_COOKIE['Cartlist'] = NULL;
			_setcookie("Cartlist",null);
