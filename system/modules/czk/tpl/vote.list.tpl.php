...
<div class="table-list lr10">
 <table width="100%" cellspacing="0">
	...
	$meminfo=$this->db->GetOne("select * from `@#_member` where uid=$v[uid]");
	?>
	<tr align="center" class="mgr_tr">
		<td ><?php echo $v['id'];?></td>
        <td ><?php if($meminfo['mobile']==""){ echo $meminfo['username'];}else{ echo $meminfo['mobile'];}?></td>
		<td ><?php echo $v['czknum'];?></td>
		<td ><?php echo $v['password'];?></td>
		<td ><?php echo $v['mianzhi'];?></td>
        
		<td ><?php if($v['type']==1){echo '一次性';}elseif($v['type']==2){echo '体验';}?></td>
        <td ><?php if($v['status']==0){echo date("Y-m-d H:i:s",$v['time']);}?></td>
        <td ><?php if($v['status']==1){echo '<font color="red">未使用</font>';}else{echo '已使用';};?></td>
