...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
   ...
   // 今日揭晓商品
	...
	public function userlogin(){
		...
		$getType = safe_replace($this->segment(6));
		...
		$this->db->GetOne("UPDATE `@#_member` SET `login_time` = '$time',`getType` = '$getType' where `$logintype`='$username'");
		if(!$member){
			//帐号不存在错误
			$user['state']=1;
			$user['num']=-2;
...
# 我的修改
		...
		$getType= isset($_GET['getType'])? $_GET['getType']: $this->segment(6);
		$time=time();
		$decode = 0;
		//邮箱验证 -1 代表未验证， 1 验证成功 都不等代表等待验证
		$sql="INSERT INTO `@#_member`(`mobile`,password,img,emailcode,mobilecode,yaoqing,time,getType)VALUES('$name','$pass','photo/member.jpg','-1','-1','$decode','$time','$getType')";
	...
	public function getLotteryList(){
	   ...
	   $shopsum=$this->db->GetList("select * from `@#_shoplist` where $wheres ");

	   //最新揭晓
		$shoplist['listItems']=$this->db->GetList("select * from `@#_shoplist` where $wheres ORDER BY `q_end_time` DESC limit $FIdx,$EIdx");
