...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserRecharge(){
	   ...
	   $sql = "`uid`='$uid' and `pay` = '账户'  and `type`='1' or `uid`='$uid' and  `pay` = '佣金' and `type`='1'";

	    $Rechargelist=$this->db->GetList("select * from `@#_member_account` where $sql  ORDER BY time DESC");

	    $Recharge['listItems']=$this->db->GetList("select * from `@#_member_account` where $sql  ORDER BY time DESC limit $FIdx,$EIdx ");
	...
	public function getUserConsumption(){
	   ...
	   $sql = "`uid`='$uid' and `pay` = '账户'  and `money` != '0'  and `type`='-1' ";

	   $Consumptionlist=$this->db->GetList("select * from `@#_member_account` where  $sql ");

	   $Consumption['listItems']=$this->db->GetList("select * from `@#_member_account` where  $sql ORDER BY time DESC limit $FIdx,$EIdx ");
       	file_put_contents("D:/test.txt","select * from `@#_member_account` where  $sql ORDER BY time DESC limit $FIdx,$EIdx ");
