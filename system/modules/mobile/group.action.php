...
class group extends base {
	...
	public function init() {
		...
		include templates("mobile/index","group");
	}
	...
	public function huatiList() {
		...
		$qz=$this->db->GetPage("select * from `@#_quanzi_tiezi` WHERE qzid='$id' and `tiezi`='0' and `shenhe` = 'Y' order by zhiding DESC,id DESC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));		
		include templates("mobile/index","huatiList");
		
	}
	
	/*话题列表*/
	public function list_ifram() {
		$yid = $this->segment(5);
		$user = $this->userinfo;
		$uid = $user['uid'];
		$uids = base64_encode($uid);
		...
		$qz=$this->db->GetPage("select * from `@#_quanzi_tiezi` WHERE qzid='$id' and `tiezi`='0' and `shenhe` = 'Y' order by zhiding DESC,id DESC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));		
		include templates("mobile/index","list_ifram");
	...
	public function nei(){
		...
		/*圈子是否能加入*/
		$tid = $tiezi['qzid'];
		if($tid==6){$othertitle="猜你喜欢";}elseif($tid==7){$othertitle="相关热门视频";}
		...
		$tieziother=$this->db->GetPage("select * from `@#_quanzi_tiezi` WHERE tiezi='0' and qzid='$tiezi[qzid]' and id!='$id' and `shenhe` = 'Y' order by id DESC",array("num"=>5,"page"=>$pagenum,"type"=>1,"cache"=>0));
		
		include templates("mobile/index","nei");
	}
	...
	public function neifu(){
		...
		$hueifu=$this->db->GetPage("select * from `@#_quanzi_tiezi` WHERE tiezi='$id' and `shenhe` = 'Y' order by id DESC",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));
		include templates("mobile/index","neifu");
	}
