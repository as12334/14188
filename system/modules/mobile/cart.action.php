...
$shoparr = $this->db->GetList ( "SELECT * FROM `@#_shoplist` where `id` in($shopids)", array ("key" => "id" ) );}
		  ...
		  if (count ( $shoplist ) >= 1) {
			  foreach ( $Mcartlist as $key => $val ) {
				  ...
				  if (isset ( $shoplist [$key] )) {
					  ...
					  if($shoplist [$key] ['yunjiage']==0){$shoplist [$key] ['cart_xiaoji'] = 1;}else{
 ...
 //修改支付每次都要使用福分问题 lq 2014-12-01
		...
		if($ok != 'ok'){
		  ...
		  if (! $check) {
			  _messagemobile ( "对不起，手慢，商品已经有人在购买了!,请<a href='" . WEB_PATH . "/mobile/cart/cartlist' style='color:#22AAFF'>返回购物车</a>查看" );
		  }
		  if ($check) {
			  // 成功
	...
	public function jf_paysubmit(){
		...
		if(!$check){
			...
			_messagemobile("对不起，手慢，没抢到!,请<a href='".WEB_PATH."/mobile/cart/jf_cartlist' style='color:#22AAFF'>返回购物车</a>查看");
		}
		if($check){
			//成功
