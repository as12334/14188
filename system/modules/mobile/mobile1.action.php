...
class mobile extends base {
	...
	public function init(){
		...
		$orderlist=$this->db->GetList("select * from `@#_shoplist` where $orders ORDER BY `shenyurenshu` ASC LIMIT 4");
	...
	public function glistajax(){
		...
		if($select == 60){
			...
			foreach($sun_cate as $v){
                ...
                if($fen1){
			$count=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid` in ($sun_id_str) $select_w limit $star,$end");
		}else{
	...
	public function item(){
		...
		if(!$itemlist[0]['q_uid']){
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li class="cur"><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."第".$itemlist[0]['qishu']."期<i></i><b></b></a></li>";
			else
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."第".$itemlist[0]['qishu']."期<i></i></a></li>";
		}else{		
			if($itemlist[0]['id'] == $item['id'])
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."第".$itemlist[0]['qishu']."期<i></i></a></li>";
			else
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/dataserver/'.$itemlist[0]['id'].'" class="gray02">第'.$itemlist[0]['qishu'].'期</a></li>';
		}
		unset($itemlist[0]);			
		foreach($itemlist as $key=>$qitem){
			...
			if($qitem['id'] == $item['id'])
				...
				$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/dataserver/'.$qitem['id'].'" class="gray02">第'.$qitem['qishu'].'期</a></li>';	
		}
...
$itemzx=$this->db->GetOne("select * from `@#_shoplist` where `sid`='$item[sid]' and `qishu`>'$item[qishu]' and `q_end_time` is null order by `qishu` DESC LIMIT 1");
	...
	public function dataserver(){
		...
		if(empty($item['q_user_code'])){
			_message("该商品正在进行中...",WEB_PATH.'/mobile/mobile/item/'.$itemid);
		}
		...
		if(empty($itemlist[0]['q_uid'])){			
			$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."第".$itemlist[0]['qishu']."期<b></b></a></li>";
			unset($itemlist[0]);
		}else{		
			$loopqishu.='<li><a href="'.WEB_PATH.'/mobile/mobile/item/'.$itemlist[0]['id'].'">'."第".$itemlist[0]['qishu']."期<b></b></a></li>";
			unset($itemlist[0]);
