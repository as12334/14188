...
class quanzi extends admin {
	...
	public function __construct(){
		...
		$this->ment=array(
			...
			array("addcate","添加帖子",ROUTE_M.'/'.ROUTE_C."/tiezi_insert"),
	...
	public function tiezi(){
		...
		$tiezi=$this->db->GetPage("select * from `@#_quanzi_tiezi` where  tiezi=0 and  `qzid`= '$qzid'",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));		
		include $this->tpl(ROUTE_M,'quanzi.tiezi');
	}
	
	/*帖子添加*/
	public function tiezi_insert(){
		$id=$this->segment(4);
		if(isset($_POST["submit"])){
			$qzid= $_POST['qzid'];
			...
			$jinghua= $_POST['jinghua'];
			$thumb = htmlspecialchars($_POST['thumb']);	
			$url= $_POST['url'];
			$time= time();
			
			$res=$this->db->GetOne("SELECT uid FROM `@#_member` WHERE `email`='$username' or `mobile`='$username'");
			if(empty($res)){
				_message("发帖子会员不存在");
			}else{
				$uid=$res['uid'];
			...
			//_message($jinhua);
			if($qzid==null || $username==null || $title==null || $neirong==null){
				_message("*号不能为空");
			}
			$this->db->Query("INSERT INTO `@#_quanzi_tiezi`(`qzid`,`hueiyuan`,`title`,`neirong`,`zuihou`,`shenhe`,`time`,`url`,`image`)VALUES('$qzid','$uid','$title','$neirong','$uid','Y','$time','$url','$thumb')");
			$tiez=$this->db->GetOne("select * from `@#_quanzi` where `id` = '$qzid'");
			$tznum=$tiez['tiezi']+1;
			$this->db->Query("UPDATE `@#_quanzi` SET `tiezi`='$tznum' where `id`='$qzid'");
			
			_message("添加成功",WEB_PATH."/group/quanzi/");
		}
		$quanzi=$this->db->GetList("select * from `@#_quanzi`");
		
		include $this->tpl(ROUTE_M,'quanzi.tiezi.insert');
	}
	/*帖子修改*/
	public function tiezi_update(){
		...
		if(isset($_POST["submit"])){
			...
			if($this->db->Query("UPDATE `@#_quanzi_tiezi` SET `title`='$title',`neirong`='$neirong',`zhiding`='$zhiding',`jinghua`='$jinghua',`url`='$url',`image`='$thumb' where `id`='$ids'")){
	...
	public function liuyan(){
		$id=$this->segment(4);
		$tiezi=$this->db->GetOne("select * from `@#_quanzi_tiezi` where tiezi=0 and id='$id' order by id desc ");
		$num=20;
		$total=$this->db->GetCount("select * from `@#_quanzi_tiezi` where tiezi!=0 and tiezi='$id' "); 
		$page=System::load_sys_class('page');
		if(isset($_GET['p'])){
			$pagenum=$_GET['p'];
		}else{$pagenum=1;}		
		...
		$hueifu=$this->db->GetPage("select * from `@#_quanzi_tiezi` where tiezi!=0 and tiezi='$id'",array("num"=>$num,"page"=>$pagenum,"type"=>1,"cache"=>0));		
