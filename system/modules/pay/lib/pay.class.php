...
class pay {
	...
	private function set_dingdan($pay_type='',$dingdanzhui=''){
			...
			$auto_user =  $this->members['auto_user'];
            			
		
			/*订单时间*/
			$time=sprintf("%.3f",microtime(true));	
			$getType = $this->members['getType'];
			$this->MoenyCount=0;
			foreach($this->shoplist as $key=>$shop){														
					...
					$codes = $ret_data['user_code'];									//得到的微购码					
					$codes_len= intval($ret_data['user_code_len']);						//得到微购码个数					
					...
					if($codes_len){
						$insert_html.="('$dingdancode','$dingdancode_tmp','$uid','$username','$uphoto','$shop[id]','$shop[title]','$shop[qishu]','$codes_len','$money','$codes','$pay_type','$ip','$status','$time','$getType','$auto_user'),";		
					}
			}			
			$sql="INSERT INTO `@#_member_go_record` (`code`,`code_tmp`,`uid`,`username`,`uphoto`,`shopid`,`shopname`,`shopqishu`,`gonumber`,`moneycount`,`goucode`,`pay_type`,`ip`,`status`,`time`,`getType`,`auto_user`) VALUES ";
			$sql.=trim($insert_html,',');				
			//$this->db->Query("set global max_allowed_packet = 2*1024*1024*10");
			
			//自动指定会员中奖
			$shopauto=$this->db->GetOne("select * from `@#_shoplist` where `id` = '$shop[id]'");
			$memauto=$this->db->GetOne("select * from `@#_member_go_record` where `auto_user` = '1' and `shopid` = '$shop[id]' order by id desc");
			$sidauto=$this->db->GetOne("select * from `@#_shoplist` where `sid` = '$shopauto[sid]' and `id` != '$shop[id]' order by id desc");
			
			$total=$this->db->GetOne("select sum(gonumber) from `@#_member_go_record` where `auto_user` = '1' and `shopid` = '$shop[id]'");
			if($shopauto['zhiding']==0 || $shopauto['zhiding']==$sidauto['zhiding']){
			
			if($total['sum(gonumber)']>$shopauto['auto_money']){
				$auto_ok = $this->db->Query("UPDATE `@#_shoplist` SET `zhiding` = '$memauto[uid]' where `id` = '$shop[id]'");
				}
			}
		//	$q1 = $this->db->Query("DELETE FROM `@#_member_go_record` WHERE `status` = '未付款,未发货,未完成'");
	...
	**/
	...
	private function pay_bag(){
		...
		$query_4 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '-1', '$pay_zhifu_name', '微购了商品', '{$this->MoenyCount}', '$time')");	
		$query_5 = true;
		$query_insert = true;
		...
		//添加微积分
		if(!$this->fufen_to_money){
			$mygoscore = $fufen['f_shoppay']*$this->MoenyCount;
			$mygoscore_text =  "微购了{$goods_count_num}人次商品";
			$myscore = $this->members['score'] + $mygoscore;		
			$query_add_fufen_1 = $this->db->Query("UPDATE `@#_member` SET `score`= '$myscore' WHERE (`uid`='$uid')");
			$query_add_fufen_2 = $this->db->Query("INSERT INTO `@#_member_account` (`uid`, `type`, `pay`, `content`, `money`, `time`) VALUES ('$uid', '1', '微积分', '$mygoscore_text', '$mygoscore', '$time')");
