...
function pay_go_yongjin($uid=null,$dingdancode=null){
	...
	$config = System::load_app_config("user_fufen",'','member');//微积分/经验/佣金
	$yesyaoqing=$db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$uid'");
	
	//第一级佣金	 
	if($yesyaoqing['yaoqing']){
		$yongjin=$config['fufen_yongjin']; //第一级佣金		
		...
		$db->Query("INSERT INTO `@#_member_recodes`(`uid`,`type`,`content`,`shopid`,`money`,`ygmoney`,`time`)VALUES('$yesyaoqing[uid]','1','$content','$val[shopid]','$y_money','$val[moneycount]','$time' )"); 	
	...
	//第一级佣金	
	//第二级佣金
	$yesyaoqing2=$db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$yesyaoqing[yaoqing]'");//第二级佣金
	if($yesyaoqing2['yaoqing']){
		$yongjin2=$config['fufen_yongjin_2']; //每一元返回的第二级佣金
		...
		$content2="第二级(第".$val['shopqishu']."期)".$val['shopname'];
		$db->Query("INSERT INTO `@#_member_recodes`(`uid`,`type`,`content`,`shopid`,`money`,`ygmoney`,`time`)VALUES('$yesyaoqing2[uid]','1','$content2','$val[shopid]','$y_money2','$val[moneycount]','$time' )"); 	
	...
	//第三级佣金
	$yesyaoqing3=$db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$yesyaoqing2[yaoqing]'");//第三级佣金
	if($yesyaoqing3['yaoqing']){
		$yongjin3=$config['fufen_yongjin_3']; //每一元返回的第三级佣金
		...
		$content3="第三级(第".$val['shopqishu']."期)".$val['shopname'];
		$db->Query("INSERT INTO `@#_member_recodes`(`uid`,`type`,`content`,`shopid`,`money`,`ygmoney`,`time`)VALUES('$yesyaoqing3[uid]','1','$content3','$val[shopid]','$y_money3','$val[moneycount]','$time' )"); 	
	...
	//第四级佣金
	$yesyaoqing4=$db->GetOne("SELECT * FROM `@#_member` WHERE `uid`='$yesyaoqing3[yaoqing]'");//第四级佣金
	if($yesyaoqing4['yaoqing']){
		$yongjin4=$config['fufen_yongjin_4']; //每一元返回的第四级佣金
		$yongjin4 = floatval(substr(sprintf("%.3f",$yongjin4), 0, -1));
		...
		$content4="第四级(第".$val['shopqishu']."期)".$val['shopname'];
		$db->Query("INSERT INTO `@#_member_recodes`(`uid`,`type`,`content`,`shopid`,`money`,`ygmoney`,`time`)VALUES('$yesyaoqing4[uid]','1','$content4','$val[shopid]','$y_money4','$val[moneycount]','$time' )"); 	
