...
class pay extends admin {
	...
	public function pay_list(){		
		...
		$paylist = $this->db->GetList("SELECT * FROM `@#_pay` where pay_id>3 order by pay_id desc ");
		$path = G_SYSTEM."modules".DIRECTORY_SEPARATOR."pay".DIRECTORY_SEPARATOR."lib".DIRECTORY_SEPARATOR;
	...
	public function pay_set(){	
		...
		if($pay['pay_class'] == 'yeepay'){
			if(!file_exists(G_SYSTEM.'modules/'.ROUTE_M.'/lib/yeepay.class.php')){
				_message("开通易宝支付请联系官网!<a href='http://www.baidu.com/'>http://www.baidu.com/</a>",'http://www.baidu.com/',10);
			}		
		}
