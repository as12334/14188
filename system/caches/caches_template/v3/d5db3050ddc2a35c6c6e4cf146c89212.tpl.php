		...
		if(data[0].sum){
			...
			for(var i=0;i<data.length;i++){
				...
				var div='<div class="cSingleInfo"><dl class="fl">';
					div+='<a href="<?php echo WEB_PATH; ?>/mobile/mobile/userindex/'+data[i].sd_userid+'/<?php echo $uids; ?>/">';
...
$(function(){
	...
	$(document).on('click',gt,function(){
		var id=$(this).attr('id');
		if(id){
			window.location.href="<?php echo WEB_PATH; ?>/mobile/shaidan/detail/"+id+"/<?php echo $uids; ?>/";
		}			
	});
	$(document).on('click','.cSingleInfo .blue',function(){
		var id=$(this).attr('id');
		if(id){
			window.location.href="<?php echo WEB_PATH; ?>/mobile/mobile/userindex/"+id+"/<?php echo $uids; ?>/";
		}	
	});	
})
</script>
