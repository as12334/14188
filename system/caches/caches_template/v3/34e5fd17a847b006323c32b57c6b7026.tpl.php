...
<body style="width:970px; min-height:35px;  padding-top:20px;background-color:#fff">
		...
		<div class="AllRecW AllReclist"><div class="AllRecL fl"><?php echo microt($user['time']); ?><i></i></div>
			...
			<p class="AllRecR_T">            	
				...
				<?php if($user['ip']): ?>
				<!--<img src="<?php echo G_TEMPLATES_STYLE; ?>/images/mobile/location-ico.png" width="13"> --><span class="address"></span>(<?php echo get_ip($user['id'],'ipcity'); ?>)
				<?php endif; ?>
...
$(function(){
	...
	$(".user_see_code").click(function(){
		if($(this).attr("see")=='off'){
			$(this).attr("see","on");
			$(this).text("关闭<?php echo _cfg('web_name_two'); ?>码");
			$(this).parents().next().show();
		}else{
			$(this).text("显示<?php echo _cfg('web_name_two'); ?>码")
			$(this).attr("see","off");
