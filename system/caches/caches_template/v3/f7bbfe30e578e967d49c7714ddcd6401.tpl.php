...
<div class="groups-right">
	...
	<div class="grhead-info">
		<p class="grhead-name"><a href="<?php echo WEB_PATH; ?>/uname/<?php echo idjia(uidcookie('uid')); ?>" class="gray" ><?php echo get_user_name(uidcookie('uid'),'username'); ?></a></p>
		<p class="grhead-topic gray02">话题<?php echo $m_huati; ?><span class="gray03"> | </span>回复<?php echo $m_hueifu; ?></p>
		<div id="divJoinGroup">
	        <p id="pJoinGroup" class="grhead-join gray02"> 加入圈子<span id="spJoinGroupNum" class="orange"><?php if(qznum()==null): ?>0<?php  else: ?><?php echo qznum(); ?><?php endif; ?></span>个<b></b></p>
	    </div>
	</div>	
	<?php  else: ?>
	...
	<div id="divHotTopic" class="groups-shadow clearfix">
		...
		<?php $tiezi=$this->DB()->GetList("select * from `@#_quanzi_tiezi`  where tiezi=0 and  zhiding!='Y' and shenhe='Y' order by dianji desc LIMIT 6",array("type"=>1,"key"=>'',"cache"=>0)); ?>
		...
		<div class="R-list Topic-list">
			...
			<div class="groups-Rinfo"><!-- <?php echo _strcut($tz['title'],24,""); ?> -->
				...
				<p class="groups-c gray02"><?php echo _strcut(quanzid($tz['qzid']),12); ?><span class="gray03"> | </span><?php echo $tzhueifu; ?>条回复</p>
			</div>
		</div>
	...
	<div class="groups-shadow clearfix">
		...
		<div class="Member-list clearfix">
			...
			<?php $ln=1;if(is_array($hueifu)) foreach($hueifu AS $hf): ?>		
				...
				<?php endif; ?>	
				<i><?php echo get_user_name($hf['hueiyuan'],'username'); ?></i></a>
				</li>		
			<?php  endforeach; $ln++; unset($ln); ?>
			<?php if(defined('G_IN_ADMIN')) {echo '<div style="padding:8px;background-color:#F93; color:#fff;border:1px solid #f60;text-align:center"><b>This Tag</b></div>';}?>
			</ul>
