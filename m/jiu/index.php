...
<body>
   ...
   <header class="head" id="head">
        <div class="fixtop">
                            <span id="t-find"><a href="<?php echo $url; ?>/m" class="btn btn-left btn-back"></a></span>
...
while($row=$db->getrow($rrip)){?>
    <li data_jstype="<?php echo $row['id'] ?>" class="box" style="overflow:hidden">        <a class="goods-a" href="<?php echo $url; ?>/m/item/?id=<?php echo $row['id'] ?>">             <?php
    if ($img_sl!=''){
        $img_sl=substr($img_sl,0,strrpos($img_sl,'/')+1).'d'.substr($img_sl,strrpos($img_sl,'/')+1);
    ?>
    <img class="lazy" _src="<?php echo $url; ?>statics/uploads/<?php echo $row['img_sl0'] ?>" src="<?php echo $url; ?>/<?php echo $img_sl; ?>" onload="if (this.width!=this.height){this.height=this.width;}" >  <?php
    }
    ?>           
