...
if(!$rowlp=$db->getrow($resultlp)){
	...
	$sqlLpAdd='select * from `'.$tablepre.'lp_address` where `lm`="'.$id.'"';
$resultLpAdd=$db->query($sqlLpAdd);
$rowLpAdd=$db->getrow($resultLpAdd);
?>
<!DOCTYPE html>
<html>
    <head>
    ...
    <div class="main">
       ...
       <table width="100%" cellpadding="0" align="center" border="0">
                                  ...
                                  <tr><td width="25%">&nbsp;<?php echo date("Y-m-d",$rowlp['ttime']);?>&nbsp;</td>
                                  <td width="25%" align="center">&nbsp;<?php echo date("Y-m-d",$rowlp['ttime']);?>&nbsp;</td>
                                  <td width="25%" align="center">&nbsp;<?php if($rowlp['types']==1){}else{ echo date("Y-m-d",$rowlp['ftime']); }?>&nbsp;</td>
                                  <td align="right">&nbsp;<?php if($rowlp['types']==3){ echo date("Y-m-d",$rowlp['stime']);}?>&nbsp;</td>
                                  </tr></table>
             </li><?php if($rowlp['types']==2 || $rowlp['types']==3){?>
                                <li class="normal">
                   
                    <?php if($rowlp['types']==3 && $rowLpAdd['gc']==''){?>
                  
                       <span class="name"> 来源：</span><span style="color:red"><?php echo $rowSeo['name'] ?>特卖使用</span>
                  
                    <?php }else{?>
                    <span class="name"> 物流信息：</span><span style="color:red"><?php echo $rowLpAdd['gc'];?>，<?php echo $rowLpAdd['dh'];?></span>
                    <?php }?>
                </li>
                <?php }?>
                <li class="normal">
                   
                     <span class="name">商品清单：</span><span style="color:red"><?php echo $rowlp['title'];?></span>
                </li>
