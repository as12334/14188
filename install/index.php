...
include(rootdir."/install/lib/Smarty/libs/Smarty.class.php");  
...
$mode = 0777;
if(file_exists(rootdir.'system/config/database.inc.php'))
{
	echo '已经安装过此程序...';
	exit;
}
    ...
    case '1': //安装许可协议 
	  ...
	  $smarty->assign("stepname", '数据导入');
		...
		if($module == 'admin') { 
			...
			$dbfile =  $tablepre;	
			if(file_exists(rootdir."db/".$dbfile)) {
				$sql = file_get_contents(rootdir."db/".$dbfile);
				_sql_execute($sql,$tablepre); 
			} else {
				echo '2';//数据库文件不存在
			}							
		}elseif($module == 'mkconfig'){
			if($fp = @fopen(rootdir."system/config/database.inc.php", 'w')) {
       @fclose($fp);    
       }else{
       	echo '创建配置文件失败请检测网站根目录下的config文件是否拥有写入功能';
       	exit;
      }  
      include(rootdir.'install/lib/core/urlmanager_class.php'); 
     
    
		$path = rootdir.'system/config/database.inc.php';
					$html=<<<HTML
<?php
