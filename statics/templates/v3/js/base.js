...
var __UPLOAD__ = 'http://'+window.location.host; //资源文件路径
var __URL_UPLOAD__ = 'http://'+window.location.host;//上传地址
var __URL_JUANPI__ = 'http://'+window.location.host;
var __URL_JKY__ = 'http://'+window.location.host;
var __URL_FANLI__ = 'http://'+window.location.host;
var __URL_BRAND__ =  'http://'+window.location.host;
var __URL_TAO__  =  'http://'+window.location.host;
var __URL_CLICK__ =  'http://'+window.location.host;
var __URL_ZHE__  =  'http://'+window.location.host;
var __URL_USER__ =  'http://'+window.location.host+'/user';
var __URL_MEMBER__   = 'http://'+window.location.host;//用户中心配置
var __URL_SHARE__ =  'http://'+window.location.host;
var __URL_HEZUO__ = 'http://'+window.location.host;
var __URL_SELLER__ = 'http://'+window.location.host;
var __URL_JU__ = 'http://'+window.location.host;
var __URL_M_JUANPI__ = 'http://'+window.location.host;//
var __URL_M_JIUKUAIYOU__ = 'http://'+window.location.host;//九块邮手机WAP
var __URL_SHOP__ = 'http://'+window.location.host;
var __URL_SHOP__ = 'http://'+window.location.host;
var __URL_CART__ = 'http://'+window.location.host; //购物车
var __URL_PAD__ = 'http://'+window.location.host; //购物车
...
XDPROFILE = {uid:__XD_USER__.uid,guestswitch:0,face:__U_UPLOAD__+__XD_USER__.pic,username:__XD_USER__.nick,sign:__XD_USER__.sign};
XDPROFILE.guestswitch = XDTOOL.empty(XDTOOL.getCookie('s_guestswitch'))?0:XDTOOL.getCookie('s_guestswitch');
...
(function($) {
    if (XDPROFILE.uid != '') {
        ...
        $.ajax({
            type: 'get',
            url: __URL_CART__+'/MiniCart/hasStock',
            dataType: 'jsonp',
            success: function(data) {
                if(data.status == 1001){
                    $('.tag_remind').show();
                    $('.tag_remind .remindnum').text(data.num);
                }
            }
        });
        
        //关闭弹窗
        $('.tag_remind a.del').live('click',function(){
            $.ajax({
                type: 'get',
                url: __URL_CART__+'/MiniCart/closeRemind',
                dataType: 'jsonp',
                success: function(data) {
                    $('.tag_remind').hide();
                }
...
//对象转时间格式,例如objectToDate(new Date(),'yyyy-MM-dd hh:mm:ss'),format不传默认就是yyyy-MM-dd hh:mm;
function objectToDate(object,format){
	...
	var date = {
       ...
       if (/(y+)/i.test(format)) {
              format = format.replace(RegExp.$1, (object.getFullYear() + '').substr(4 - RegExp.$1.length));
       }
       for (var k in date) {
...
//时间戳转时间格式,例如UnixToDate(1403149534,'yyyy-MM-dd hh:mm:ss'),format不传默认就是yyyy-MM-dd hh:mm;
function UnixToDate(unixTime,format) {
	   format = format == undefined?"yyyy-MM-dd hh:mm":format;
	   var object = new Date(unixTime * 1000);
       return objectToDate(object,format);
}
