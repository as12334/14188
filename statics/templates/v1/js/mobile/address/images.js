...
var __HOST_IMAGE__ = Config.cloud == 'juancdn' ? '//s1.juancdn.com' : '//s2.juancdn.com',
    ...
    __HOST_FILE__,
    __HOST_SECRET__,
    __ACT_STATIC__='//act.juancdn.com',
    __HOST_STATIC__='//s.juancdn.com';
...
Images.prototype.parseSrc = function (path) {
    var patt = new RegExp('((https?:)?\\/\\/.*?)?\\/?.*?_(\\d+)x(\\d+)\.([a-z0-9]{2,6})');
    ...
    if (!this.empty(matches)) {
        ...
        if (!this.empty(matches[1])) {
            ...
            info.path = info.path.replace(patt2,"/");
        }
        if (info.path[0] !== "/") {
            info.path = "/" + info.path;
        }
...
function getOriginalSrc (oUrl) {
    if(oUrl==''){ 
        return '';
    }else {
        var suffixArry=['.jpg', '.jpeg', '.png', '.gif', '.bmp', '.ico', '.webp'];
        var index=-1;
        var new_url='';
        for (var i =0;i<suffixArry.length;i++){
            var currentSuffix=suffixArry[i];
            index=oUrl.indexOf(currentSuffix);
            if(index>=0){
                index=index+parseInt(currentSuffix.length);
                new_url=oUrl.substr(0,index);
                return new_url;
            }
        }
        return new_url;
...
function getResizeImageUrl(path, width, height, quality, suffix, host) {
    ...
    if (undefined != matches) {
        ...
        if (date > 151201) {
            config = "ucloud";
            if (undefined != host) {
                patt = new RegExp('(https?:)?\\/\\/.*');
                matches = patt.exec(host);
...
function getImageUrl(path) {
    ...
    ext = path.slice(index + 1);

    return getResizeImageUrl(path, 0, 0, 100, ext);
}
 ...
 * @param quality
 */
function getImageUrlBySize(path, sizeName, quality)
{
    var sizes = {
        's': [90, 90],
        'm': [180, 180],
    ...
    index = path.lastIndexOf('.');
    ext = path.slice(index + 1);
    return getResizeImageUrl(path, size[0], size[1], quality || 88, ext);
}